...
- Manejo de error del backend (respuesta 500 al registrar).
...
- Los casos de "registro exitoso" y "error del servidor" mockean las respuestas del backend (`/api/dashboard/register/*/validate` y `/api/dashboard/register/`) para no crear cuentas reales en producción en cada corrida: `mockSuccessfulFlow(user)` mockea ambos endpoints juntos para el caso exitoso, y `mockFailedRegister()` simula un error del servidor. Además, `blockRegisterApi()` corre en un `beforeEach` global que corta cualquier POST real a `/api/dashboard/register/` como red de seguridad, incluso si algún test nuevo se olvida de mockear.
...
- **Bug**: cuando `/api/dashboard/register/{email}/validate` responde `valid_email:false` (por ejemplo, un email ya registrado), el formulario lo ignora por completo: no muestra ningún error en el campo de Corporate Email y el registro avanza igual hasta la pantalla de éxito. También documentado con `test.fixme`.
